...
@section('breadcrumb_buttons')
    <a href="{{ route('administration.league.show', ['league' => $league]) }}"
        class="btn btn-outline-dark btn-outline-custom fw-bolder">
...
@section('content')
    ...
    <div class="row justify-content-center">
                            ...
                            <table class="table table-bordered mb-0">
                                <tbody>
                                    <tr class="text-center">
                                        <td colspan="2">
                                            <div class="user-avatar">
                                                <img src="{{ show_image($league->logo) }}" alt="League Logo"
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('administration.league.show', ['league' => $league]) }}"
                                                ...
                                                title="Click to see {{ $league->name }} details"
                                    ...
                                    <tr>
                                        <th>Season</th>
                                        <td>
                                            <a href="{{ route('administration.season.show', ['season' => $league->season]) }}"
                                                target="_blank" data-toggle="tooltip" data-placement="top"
                                                title="Click to see {{ $league->season->name }} details"
                                                class="text-dark text-bold">
                                                {{ $league->season->name }}
                                            </a>
                                        </td>
                                    ...
                                    <tr>
                                        <th>Sport</th>
                                        <td>
                                            <a href="{{ route('administration.sport.show', ['sport' => $league->sport]) }}"
                                                target="_blank" data-toggle="tooltip" data-placement="top"
                                                title="Click to see {{ $league->sport->name }} details"
                                                class="text-dark text-bold">
                                                {{ $league->sport->name }}
                                            </a>
                                        </td>
        ...
        <div class="col-md-6">
            @if (Session::has('league_register'))
                <!-- Start col -->
                <div class="col-md-12">
                    <div class="alert-list">
                        <div class="alert alert-success" role="alert">
                            <h4 class="alert-heading">Congratulations! </h4>
                            <p>Your payment has been successfully processed, and your child is officially
                ...
                <!-- End col -->
            @else
                <form action="{{ route('administration.league.registration.store', ['league' => $league]) }}"
                    method="post" class="card-form" enctype="multipart/form-data" autocomplete="off">
                    @csrf
                    @role('player')
                            ...
                            <div class="row">
                                <div class="form-group col-md-12">
                                    @role('admin|developer|guardian')
                                        ...
                                        <select class="select2-single form-control @error('player_id') is-invalid @enderror"
                                            name="player_id" required>
                                            <option value="">Select Player</option>
                                            @foreach ($players as $player)
                                                <option value="{{ $player->id }}" @selected(old('player_id') == $player->id)>
                                                    {{ $player->user->name }}</option>
                                            @endforeach
                                        </select>
                                    @endrole
                                    @error('player_id')
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="form-row">
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="text" class="form-control" value="{{ old('card_number') }}"
                                                name="card_number" id="cardnumber" required>
                                        </div>
                                        <div class="form-group col-md-6">
                                            <label for="cardfullname">Full Name</label>
                                            <input type="text" class="form-control"
                                                value="{{ old('card_holder_name') }}" name="card_holder_name"
                                                id="cardfullname" required>
                                        </div>
                                    </div>
                                    <div class="form-row">
                                        <div class="form-group col-md-6">
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="text" class="form-control" value="{{ old('card_cvc') }}"
                                                name="card_cvc" id="cardcvc" required>
                                        </div>
                                    </div>
                                </div>
                            </div>
