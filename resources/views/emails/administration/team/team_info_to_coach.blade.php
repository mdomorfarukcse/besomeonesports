...
<div>
    ...
    <br>
    You have been assigned as a Coach in a New Team called <a href="{{ route('administration.team.show', ['team' => $data]) }}"><strong>{{ $data->name }}</strong></a>. Your team Information has been given below.
</div>
<br>
<table cellpadding="0" cellspacing="0" style="width: 100%; border: 1px solid #ededed;">
    <tbody>
        ...
        <tr>
            ...
            <td style="padding: 10px; border-bottom: 1px solid #ededed; color: #455056;">
                <a href="{{ route('administration.division.show', ['division' => $data->division]) }}">
                    <strong>{{ $data->division->name }}</strong>
                </a>
            </td>
        </tr>
    </tbody>
